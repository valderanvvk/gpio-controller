...
describe('GpioMock edge functions', () => {
	it('[ edge() ] Checking edge with direction SUCSESS', () => {
		...
		const gpio1 = new GpioMock(12, 'out', 'both');
		const gpio2 = new GpioMock(13, 'in', 'both');
		const gpio3 = new GpioMock(14, 'in', 'falling');
		const gpio4 = new GpioMock(2, 'in', 'rising');
		const gpio5 = new GpioMock(3, 'in');
	...
	it('[ setEdge() ] Checking setEddge with direction SUCSESS', () => {
		...
		const gpio = new GpioMock(12, 'out', 'falling');
